...
const TokenSelectorModal = ({ open, setOpen, selectedToken }: Props) => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="p-0 pb-6 sm:max-w-lg">
        ...
        <div className="max-h-[60vh] overflow-y-auto">
          {loading ? (
            ...
            filteredTokens.map((token, index) => (
              <button
                key={index}
                className="flex items-center w-full px-6 py-3 gap-x-4 hover:bg-secondary disabled:pointer-events-none disabled:opacity-50"
